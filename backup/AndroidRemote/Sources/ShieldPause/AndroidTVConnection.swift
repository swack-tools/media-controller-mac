...
class AndroidTVConnection {
    ...
    func receiveMessage() async throws -> Data {
        ...
        var shift = 0
        while true {
            let byte = try await receive(minLength: 1, maxLength: 1)
            guard let b = byte.first else {
        ...
        return try await receive(minLength: length, maxLength: length)
    }
